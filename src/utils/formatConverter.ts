...
export function parseUstText(text: string): ProjectData {
  ...
  for (let line of lines) {
    ...
    if (line.startsWith('[#SETTING]')) {
      ...
      if (inNote) {
        // Save previous note if not rest
        const isRest = curLyric === 'R' || curLyric === 'r' || curLyric === '休符' || curLyric === 'ー';
        if (!isRest) {
          notes.push({
        ...
        currentTick += curLength;
      }
      inSetting = false;
      inNote = !line.startsWith('[#VERSION') && !line.startsWith('[#TRACKEND]');
    ...
    if (inSetting) {
      if (key.toLowerCase() === 'tempo') {
        const parsedTempo = parseFloat(val);
        if (!isNaN(parsedTempo) && parsedTempo > 0) tempo = parsedTempo;
      } else if (key.toLowerCase() === 'projectname') {
        projectName = val;
      } else if (key.toLowerCase() === 'voicedir' || key.toLowerCase() === 'voicebank') {
        voicebank = val;
      }
    } else if (inNote) {
      switch (key.toLowerCase()) {
        case 'length':
          curLength = parseInt(val, 10) || 480;
          break;
        case 'lyric':
          curLyric = val;
          break;
        case 'notenum':
          curNoteNum = parseInt(val, 10) || 60;
          break;
        case 'intensity':
          curIntensity = parseInt(val, 10) || 120;
          break;
        case 'flags':
          curFlags = val;
          break;
